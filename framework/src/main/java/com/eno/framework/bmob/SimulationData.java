...
public class SimulationData {
    ...
    public static void testData() {
        for (int i = 0; i < mGirlPhone.length; i++) {
            testData(mBoyPhone[i], mBoyName[i], mBoyPhoto[i], 0, mBoyDesc[i], mBoyAge[i], mBoyBirthday[i], mBoyConste[i], mBoyHobby[i], mBoyStatus[i]);
            testData(mGirlPhone[i], mGirlName[i], mGirlPhoto[i], 1, mGirlDesc[i], mGirlAge[i], mGirlBirthday[i], mGirlConste[i], mGirlHobby[i], mGirlStatus[i]);
        }
    }

    //因为拿的是我们本地对象模拟，所以需要清除数据
    private static void testData(String phone, String nickName, String photoUrl, int sex, String desc, int age, String birthday, String constellation, String hobby, String status) {
